...
def gather_bm25_scores(bm25_score_store, query_ids, candidate_ids, device):
    if bm25_score_store is None:
        return None

    query_indices = torch.tensor(
        [bm25_score_store['query_to_index'][case_id] for case_id in query_ids],
        dtype=torch.long,
    )
    candidate_indices = torch.tensor(
        [bm25_score_store['doc_to_index'][case_id] for case_id in candidate_ids],
        dtype=torch.long,
    )
...
def log_fusion_weights(model, writer, epoch):
    ...
    writer.add_scalar('Fusion/fact_weight', view_weights[0].item(), epoch)
    writer.add_scalar('Fusion/issue_weight', view_weights[1].item(), epoch)
    writer.add_scalar('Fusion/graph_weight', retrieval_weights[0].item(), epoch)
    writer.add_scalar('Fusion/bm25_weight', retrieval_weights[1].item(), epoch)


def forward(data, model, device, writer, dataloader, sumfact_pool_dataset, referissue_pool_dataset, label_dict, yf_path, epoch, temp, bm25_hard_neg_dict, hard_neg_num, pos_aug, ran_aug, aug_edgedrop, aug_featmask_node, aug_featmask_edge, train_flag, embedding_saving, optimizer=None, bm25_score_store=None):
    if train_flag:
        ## Training
        model.train()
        optimizer.zero_grad()
        label_width = len(next(iter(label_dict)).split('.')[0])
        log_fusion_weights(model, writer, epoch)
        ...
        for batched_graph, labels in tqdm(dataloader):
            ...
            if pos_aug:
                ...
                l_aug_pos_graph = model.combine_fact_issue_scores(l_aug_fact, l_aug_issue)
                l_aug_pos = model.combine_with_bm25(
                    l_aug_pos_graph,
                    gather_bm25_scores(bm25_score_store, batched_case_list, positive_case_list, device),
                )
            ...
            if ran_aug:  
                ...
                l_que_ran_neg_graph = model.combine_fact_issue_scores(l_que_ran_neg_fact, l_que_ran_neg_issue)
                l_que_ran_neg = model.combine_with_bm25(
                    l_que_ran_neg_graph,
                    gather_bm25_scores(bm25_score_store, batched_case_list, ran_neg_case_list, device),
                )
            ...
            print('Fact/Issue weights:', model.get_view_weights().detach().cpu().tolist())
            print('Graph/BM25 weights:', model.get_retrieval_weights().detach().cpu().tolist())
