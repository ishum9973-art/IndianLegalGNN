...
class EUGATGNN(nn.Module):
    def __init__(self, in_dim, h_dim, out_dim, dropout, num_head):
        ...
        self.view_weight_logits = nn.Parameter(th.zeros(2))
        self.retrieval_weight_logits = nn.Parameter(th.zeros(2))
    ...
    def get_view_weights(self):
        return th.softmax(self.view_weight_logits, dim=0)

    def get_retrieval_weights(self):
        return th.softmax(self.retrieval_weight_logits, dim=0)

    def combine_fact_issue_scores(self, fact_scores, issue_scores):
        weights = self.get_view_weights()
        return weights[0] * fact_scores + weights[1] * issue_scores

    def combine_with_bm25(self, graph_scores, bm25_scores=None):
        normalized_graph_scores = (graph_scores + 1.0) / 2.0
        if bm25_scores is None:
            return normalized_graph_scores

        weights = self.get_retrieval_weights()
        return weights[0] * normalized_graph_scores + weights[1] * bm25_scores
