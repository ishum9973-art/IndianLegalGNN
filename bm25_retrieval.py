...
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR) if os.path.basename(SCRIPT_DIR) == "BM25" else SCRIPT_DIR
...
def main():
    ...
    parser.add_argument(
        ...
        help="Deprecated compatibility flag; year-filtering after ranking is disabled by default.",
    )
    ...
    args.query_dir = resolve_input_path(args.query_dir)
    args.doc_dir = resolve_input_path(args.doc_dir)
    args.labels = resolve_input_path(args.labels)
    args.candidates = resolve_input_path(args.candidates)
    ...
    for query_id in label_dict.keys():
        ...
        ranked = [c for c in ranked if c != query_id]
    ...
    log_lines += [
        ...
        f"P@{args.topk}:  {_to_float(p_score)}",
    ]
    ...
    if args.candidates:
        log_lines += [
            ...
            f"P@{args.topk} yf:  {_to_float(p_score_yf)}",
        ]

    print("BM25 evaluation full log")
